...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(oper,operands,env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if i!=j:
            raise SchemeError("No of formals do not match actual args")
        else:
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        child=Frame.make_child_frame(self.env,self.formals,args)
        return child
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        expr=Pair(formals,expressions.second)
        proc_ins=do_lambda_form(expr,env)
        env.define(target.first,proc_ins)
        return target.first
...
def do_and_form(expressions, env):
    ...
    while expressions.second!=nil:
        if scheme_falsep(scheme_eval(expressions.first,env)):
            #print("expr first first",expressions.first)
            return  scheme_eval(expressions.first,env)
        else:
            expressions=expressions.second
    return scheme_eval(expressions.first,env)
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)
    child=Frame.make_child_frame(env,formals,args)
    return child
...
class MuProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        of values, for a lexically-scoped call evaluated in environment ENV."""
        # BEGIN PROBLEM 12
        child=Frame.make_child_frame(env,self.formals,args)
        return child
